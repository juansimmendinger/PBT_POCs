 ...
 */
contract PBTSimple is Initializable, ERC721ReadOnly, IPBT {
    function initialize(string memory name_, string memory symbol_) public override initializer {
        ERC721ReadOnly.initialize(name_, symbol_);
    }
    ...
    function _getChipAddressForChipSignature(
        bytes calldata signatureFromChip,
        uint256 blockNumberUsedInSig
    ...
    function _getTokenDataForChipSignature(
        ...
        address chipAddr = _getChipAddressForChipSignature(
            signatureFromChip,
            blockNumberUsedInSig
        );
        console.log(chipAddr);
        TokenData memory tokenData = _tokenDatas[chipAddr];
        if (tokenData.set) {
            return tokenData;
        }
        console.log(tokenData.set);
